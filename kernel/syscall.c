...
// you need to add an array  of syscall names to index into.
...
void syscall(void)
{
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num])
  {
    ...
    // lab-2 activity-1 modify the syscall() function to print the trace output
    int mask = p->mask;
    if ((mask >> num) & 1)
    {
      printf("%d: syscall %s -> %d\n", p->pid, sysCallName[num], p->trapframe->a0); // pid, name, return_value
    }
  }
  else
  {
